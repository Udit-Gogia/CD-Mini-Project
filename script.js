...
window.iterate = function () {
  if (iterNo >= changes.length - 1) return;
...
window.evaluate = function () {
  ...
  setTimeout(loop, 1500); // loop will start 1 second later
};
window.evaluate();

function isALetter(charVal) {
  if (charVal.toUpperCase() != charVal.toLowerCase()) return true;
  return false;
}
...
function infix2postfix(infix) {
  ...
  for (var i = 0; i < infix.length; i++) {
    ...
    if (isALetter(char)) {
      changes.push({
        stack: stack.clone(),
        output: output,
        desc: "read letter '" + char + "' and output",
      });
      output += char;
    } else if (char == "(") {
      changes.push({
        stack: stack.clone(),
        output: output,
        desc: "read '(' and push to stack",
      });
      stack.push(char);
    } else if (operators[char] > 0) {
      if (stack.length == 0 || operators[char] > operators[stack.last()]) {
        // higher precedence compared to stack, push it
        changes.push({
          stack: stack.clone(),
          output: output,
          desc: "read operator '" + char + "' and push to stack",
        });
        stack.push(char);
      } else {
        while (operators[char] <= operators[stack.last()]) {
          ...
          changes.push({
            ...
            desc:
              "read operator'" +
              char +
              "'<br>exists higher or equal precedence operator '" +
              stack.last() +
              "' pop it and output",
          });
          output += stack.pop();
      ...
      changes.push({
        stack: stack.clone(),
        output: output,
        desc: "read ')' and pop from stack until '('",
      });
  ...
  while (stack.length > 0) {
    // remaining items in stack will be automatically outputted
    changes.push({
      stack: stack.clone(),
      output: output,
      desc: "Input is empty, pop all until stack is empty",
    });
    output += stack.pop();
  }
...
function showStack(stack) {
  ...
  for (var i = 0; i < stack.length; i++) {
    new PointText({
      position: [fr.p, fr.b - 10 - 40 * i],
      fontSize: "30px",
      content: "" + stack[i],
    });
    new Path.Line({
      from: [fr.l, fr.b - 40 * i],
      to: [fr.l + fr.w, fr.b - 40 * i],
      strokeColor: "black",
      strokeWidth: 2,
    });
  }
